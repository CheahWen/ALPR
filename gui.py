...
class PopupMessage (Popup):
    def __init__(self, message):
        ...
        self.title="Error"
        error_msg=f"{message} Check the log file at LogFiles/Log.{date.today()}.log"
        self.error_msg_label.text=error_msg
class GUI(GridLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.had_informed_init_error=False
        ...
        try:
            ...
            try:
                ...
                LogManager.makeLog(message="Successfully connected to MySQL DB", type=0)
    ...
    def exit_program(self):
        ...
        if hasattr(self.db, 'pool') and self.db.pool != None:
            self.db.close_pool()
    ...
    def update(self, dt):
        try:
            ...
            if self.init_error and not self.had_informed_init_error:
                self.fire_popup(self.init_error_msg)
                self.had_informed_init_error=True
            ...
            if False:
                ...
                if frame is not None and len(frame)>0:
                    ...
                    # display image from the texture
                    self.videoPlaceholder.source = VideoPlaceHolderSource
                    self.fire_popup("Could not get video stream!")
