...
lp_det_weights_path, lp_det_input_size_x, lp_det_input_size_y, lp_det_iou_threshold, lp_det_score_threshold = ConfigParserManager.getLPDetectionConfig()
...
class PlateDetector:
    ...
    def __init__(self,
                 weights_path: str = lp_det_weights_path,
                 input_size_x: int = lp_det_input_size_x,
                 input_size_y: int = lp_det_input_size_y,
                 iou: float = lp_det_iou_threshold,
                 score: float = lp_det_score_threshold):
    ...
    def preprocess(self, frame):
        ...
        '''
        image_data = cv2.resize(frame, (self.input_size_x, self.input_size_y))
    ...
    def get_cropped_lp(self, img, bboxes):
        lp_rec=[]
        for x1, y1, x2, y2, score in self.yield_coords(img, bboxes):
            lp_rec.append([x1, y1, x2, y2, score])
        return lp_rec
   
    def draw_bboxes(self, clean_frame: np.ndarray, drawn_frame: np.ndarray, bboxes: list, display_list, mostrar_score: bool = False):
        '''
        ...
        for x1, y1, x2, y2, score in self.yield_coords(clean_frame, bboxes):
            fontScale = 2
            cv2.rectangle(drawn_frame, (x1, y1), (x2, y2), (36, 255, 12), 2)
            cv2.putText(drawn_frame, f'{score:.2f}%', (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX,
